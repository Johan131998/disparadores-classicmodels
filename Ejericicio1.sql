...
DELETE FROM productlines WHERE productLine = 'Aviones';
...
CREATE TRIGGER before_product_delete
    ...
    FOR EACH ROW
BEGIN
    INSERT INTO productsArchives(productCode, productName, productLine, productScale, 
    productVendor, productDescription, quantityInStock, buyPrice, MSRP)
    VALUES (OLD.productCode, OLD.productName, OLD.productLine, OLD.productScale, OLD.productVendor, 
    OLD.productDescription, OLD.quantityInStock, OLD.buyPrice, OLD.MSRP);
END$$
DELIMITER ;
